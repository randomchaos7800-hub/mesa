...
This document describes both MESA evaluation paths:

- `schema v2`: official benchmark methodology
- `schema v1`: legacy compatibility methodology
...
## Official v2 Methodology

The official benchmark methodology is `schema v2`.

It evaluates three observable stages:
...
The official metric families are:

- storage recall/precision and forbidden-hit metrics
- retrieval recall/precision and forbidden-hit metrics
- answer correctness, groundedness, unsupported-claim, and abstention metrics

These metrics are reported per item and in run-level summaries.

### Official v2 rationale

The benchmark is structured this way because end-to-end correctness alone is too
easy to game. A system can guess correctly, retrieve the wrong evidence, or mix
refusal language with hallucinated content. The official path is designed to
make those failures visible.

## Shared Pipeline Under Test
...
## Legacy v1 Methodology

The legacy methodology remains available for backwards comparison with older
MESA runs, but it is not the official benchmark definition.
...
The legacy LLM judge is advisory and should not be treated as an official
benchmark-grade metric.
...
### Official gold dataset (`dataset/mesa_v2.json`)
The official v2 gold dataset uses structured annotations for gold memory,
required facts, forbidden facts, and typed answer expectations.

### Legacy gold dataset (`dataset/mesa_v1.json`)
...
- **The official v2 dataset is still small.** The current observable framework is stronger than the current curated gold-set scale. Dataset growth and governance remain open work.

## Versioning

Both benchmark paths are versioned:

- `dataset/schema.json` for legacy v1
- `dataset/schema_v2.json` for official v2

Breaking changes to item format require a new schema version and migration
support. Official benchmark reporting should always include the dataset path and
dataset version so results are traceable.
